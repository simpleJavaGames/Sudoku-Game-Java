...
public class sudokuCell{
    ...
    //the boolean value is if the user got the answer correct or not.
    public boolean setUserNum(int num){
        if(isGiven){
            System.out.println("The value is already given.");
            userNum.setVisible(false);
            isUserNumVisible = false;
            return true; //Can't update something that's given, and the user does not lose any lives.
        }
        if(isActualNumLabelVisible){
            System.out.println("The actual num is visible, cannot show userNum");
            userNum.setVisible(false);
            isUserNumVisible = false;
            return true; //we already have the answer.
        }
